...
FILE=run_rawDataCalo.sh

INPUTFILE=$(python -c "import glob;l=glob.glob('*_ESD.root');print(l)")
echo [$SECONDS] INPUTFILE  = $INPUTFILE
OUTPUTFILE=rawdatacalo.root
echo [$SECONDS] OUTPUTFILE = $OUTPUTFILE
LOGFILE=runRawDataCalo.stdouterr
...
athena --command "filelist=$INPUTFILE;rootFilename='$OUTPUTFILE';evtmax=-1" /gpfs/mira-home/parton/git/btaggingML/src/RawData/share/runRawDataCalo.py
EXIT_CODE=\$?
